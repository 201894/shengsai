...
void kalman1_init(kalman1_state *state, float init_x, float init_p)
{
    ...
    state->A = 1;
    state->H = 1;
    state->q = 0.8;//0.2  10e-6;  /* predict noise convariance */
    state->r = 50;//10e-5;  /* measure error convariance */
}

void kalman13_init(kalman1_state *state, float init_x, float init_p)
{
    state->x = init_x;
    state->p = init_p;
    state->A = 1;
    state->H = 1;
    state->q = 0.2;//10e-6;  /* predict noise convariance */
    state->r = 50;//10e-5;  /* measure error convariance */
}
